...
void LeastSquareMethod::Run()
{
	...
	// расположились обратные дисперсии каждого измерени€ (размерности 
	// (N + 1) * l x (N + 1) * l), и перемножить всЄ это.
	// Ќе забываем про итерации и получение результата
}
...
MatrixXd LeastSquareMethod::SelectOnlyObservedTimeMoments(const MatrixXd & reference_trajectory) const
{
	...
	// TODO: считаем, что опорна€ траектори€ строитс€ из первого момента времени
	// наблюдений
	...
	for (size_t i = 0; i < observation_sessions_vec.size(); i++)
	{
		...
		{
			// считаем моменты времени равноотсто€щими и совпадающими
...
MatrixXd LeastSquareMethod::EvalH(const MatrixXd& reference_trajectory)
{
	...
	// TODO: вычисл€ем размер вектора измерений (считаетс€, что
	// первым столбцом идЄт врем€, а оно нам не нужно, поэтому и вычтем 1)
	...
	for (size_t i = 0; i < num_of_observations; i++)
	{
		for (size_t j = 0; j < initial_condition_size; j++)
		{
			...
			{
				// вектор-столбец частн. производной от измер€емых параметров
		...
		// TODO: по идее, присваиваем значени€ очередной матрицы H_i,
		// котора€ состоит из баллистических производных. размерности должны
		// совпадать с выдел€емым блоком
...
VectorOfMatrix LeastSquareMethod::EvalPartDerivateFromInitial()
{
	...
	for (size_t j = 0; j < initial_condition_size; j++)
	{
		...
		// каждый из var_traectories состоит только из компонент вектора состо€ни€, врем€ отсутствует
...
{
	// TODO: компоненты скорости мы не вычисл€ем
	...
	for (size_t i = 0; i < num_of_observations; i++)
	{
		...
		{
			delta(k) = var_state_vector(k) * 1.0e-3;	// делаем вариации на 3-4 пор€дка меньше в обе стороны
